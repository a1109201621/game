...
- 每次叙事 400~800 字`;
        ...
        async requestAIResponse(userMessage, isStart = false, opts = {}) {
            ...
            try {
                ...
                // dzmm API 不支持 system 角色，用 user+assistant 对模拟系统提示
                const systemPromptContent = this.createSystemPrompt();
                const messages = [
                    { role: 'user', content: `[系统设定]\n${systemPromptContent}\n[系统设定结束]\n请严格按照以上设定进行游戏。` },
                    { role: 'assistant', content: '好的，我已理解所有设定和格式要求，将严格按照规则进行游戏叙事。' },
                ...
                if (isStart) {
                    ...
                    // hideUser 时也用 user 角色发送（dzmm API 不支持 system 角色）
